...
import useAdminMenuItem from "@/hooks/useAdminMenuItems";
...
const MobileAdminDrawer = () => {
  ...
  return (
    <>
      <Link href={`/admin`} className=" w-full flex justify-start">
        <img src="/logo.png" className=" w-40 h-fit object-fill" alt="" />
      </Link>
            ...
            {menuItem?.submenus && selectedSubMenu === menuItem?._id ? (
              <div className="pl-4 mt-2">
                {menuItem?.submenus.map((submenuItem) => (
                  <div
                    key={submenuItem?._id}
                    className={`w-full flex items-center px-4 py-3 text-gray-200 hover:text-primary hover:bg-primary/10 transition-all duration-150 ease-in-out cursor-pointer ${
