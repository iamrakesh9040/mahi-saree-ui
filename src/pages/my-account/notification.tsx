...
const NotificationPage: React.FC<HeadingWithButtonProps> = () => {
  const { data, isValidating, mutate } = useSwr(`notification`);
  ...
  const handleDeleteAll = async () => {
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const res = await mutation(`notification/delete-all`, {
            method: "DELETE",
          });
          if (res?.results?.success) {
            Swal.fire({
  ...
  return (
    <PublicLayout>
      <AccountLayout>
        <div className="relative h-full  flex flex-col w-full gap-3 ">
