...
const EditAlertModal = ({ openDialog, setOpenDialog, mutate, item }: any) => {
  ...
  const handleOperation = async (
    ...
    try {
      const res = await mutation(`alert/${item?.id}`, {
        method: "PUT",
        body: {
          title: values?.title,
        },
    ...
    <>
      <div className="w-fit">
        <Dialog open={openDialog} maxWidth="sm">
          <div className="lg:w-[30rem] w-[20rem] bg-white rounded-xl flex flex-col gap-5 p-5">
            <div className="flex justify-between p-2">
              <p className="text-xl font-semibold text-gray-800">Update Form</p>
              <p
