...
const FacebookForm = () => {
  ...
  const initialValues: { [key: string]: string } = FacebookSchema.reduce(
  ...
  const handleOperation = async (values: any) => {
    ...
    try {
      const res = await mutation(`socialMediaLinks/facebook`, {
        method: "POST",
        body: {
          link: values?.link,
          type: values?.name,
      ...
      >
        {(formik) => (
          <Form className="grid grid-cols-12 gap-2 md:gap-4 w-full">
            {FacebookSchema.map((inputItem: any) => (
