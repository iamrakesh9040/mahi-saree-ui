...
const ManageProductCard = ({ isShowEdit }: { isShowEdit: boolean }) => {
  ...
  return (
    <>
      ...
      {PRODUCT_ARR.slice(0, 10).map((item) => (
        <div
          className="relative h-96 group overflow-hidden  w-full flex flex-col gap-2 justify-between items-center  bg-white shadow-[0px_0px_4px_0px_#00000024] rounded-lg p-4"
          key={item?.id}
        >
          ...
          <div className=" w-full flex  flex-col gap-4">
            ...
            <p className="  text-green-500 text-xs font-semibold">Instock</p>
              ...
              <Link href={`/panel/admin/products/manage-products/${item.id}`}>
                <p className="font-semibold tracking-wider cursor-pointer btn-secondary  duration-500 text-white  w-full text-center py-2 px-3 rounded-lg  text-sm ">
                  View Variants
                </p>
              </Link>
            </div>
