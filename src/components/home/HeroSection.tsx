...
const HeroSection = () => {
  ...
  const { data: bannerData, mutate, isValidating } = useSwr(`banner`);

  return (
    <section className="w-full lg:h-[75vh] h-[30vh] overflow-hidden relative">
      <p
        onClick={handlePrev}
      ...
      <aside className="w-full overflow-hidden z-0 relative">
          ...
          {isValidating ? (
            ...
            bannerData?.map((item: any) => (
              <img
                key={item.id}
                src={item.image}
                className="lg:h-[75vh] h-[30vh] w-full object-cover"
                alt=""
              />
            ))
          )}
        ...
        <div className="absolute bottom-5 left-1/2 transform -translate-x-1/2 flex space-x-2">
          {bannerData?.map((_: any, index: number) => (
            ...
            >
              <div
                className={`absolute top-0 left-0 h-1 rounded-full bg-primary transition-all duration-[3000ms] ${
                  currentSlide === index && animationTrigger ? "w-full" : "w-0"
                }`}
              ></div>
            </div>
          ))}
